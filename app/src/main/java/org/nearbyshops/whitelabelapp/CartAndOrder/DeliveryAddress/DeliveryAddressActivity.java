...
public class DeliveryAddressActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener,
        ViewHolderDeliveryAddressEditable.ListItemClick, View.OnClickListener,
        ...
        call.enqueue(new Callback<List<DeliveryAddress>>() {
            ...
            public void onResponse(Call<List<DeliveryAddress>> call, Response<List<DeliveryAddress>> response) {
                ...
                if(response.code()==200)
                {
                    ...
                    {
                        dataset.add(new ViewHolderButton.ButtonData("Add new address",R.drawable.ic_add_white_24px,1));
                        dataset.addAll(response.body());

                        if(response.body().size()==0)
                        {
                            dataset.add(ViewHolderEmptyScreenFullScreen.EmptyScreenDataFullScreen.emptyScreenDeliveryAddress());
                        }
                    }
                    else
                    {
...
//                    showToastMessage("Failed Code : " + response.code());
                    dataset.add(ViewHolderEmptyScreenFullScreen.EmptyScreenDataFullScreen.getErrorTemplate(response.code()));
                }
...
//        PrefLocation.saveLatLonSelected(deliveryAddress.getLatitude(),deliveryAddress.getLongitude(),this);
//        PrefLocation.setLocationSetByUser(true,this);
