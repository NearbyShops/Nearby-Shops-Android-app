...
public class ViewHolderShopSmall extends RecyclerView.ViewHolder implements PopupMenu.OnMenuItemClickListener {
    ...
    public static ViewHolderShopSmall create(ViewGroup parent, Context context, Fragment fragment,
                                             RecyclerView.Adapter adapter, int layoutType)
    {
        View view = null;

        if(layoutType==LAYOUT_TYPE_SIDE_BY_SIDE)
        {
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_shop_new,parent,false);
        }
        else if(layoutType==LAYOUT_TYPE_UBER_EATS)
        {
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_shop_uber_eats_card,parent,false);
        }
...
//        context.startActivity(shopHomeIntent);
    ...
    public void setItem(Shop shop, boolean adminModeEnabled)
    {
...
//            delivery.setText(shop.getShopAddress());
            ...
            String imagePath = PrefGeneral.getServerURL(context) + "/api/v1/Shop/Image/nine_hundred_"
            ...
            if(!shop.isRt_is_delivering())
            {
                ColorMatrix matrix = new ColorMatrix();
                matrix.setSaturation(0);
                shopLogo.setColorFilter(new ColorMatrixColorFilter(matrix));


                shopName.setText("Currently Not Delivering");
                shopName.setTextColor(ContextCompat.getColor(context,R.color.red));
            }
...
//                rating_count.setVisibility(View.VISIBLE);
                rating.setBackground(ContextCompat.getDrawable(context, R.drawable.button_color_yellow));
