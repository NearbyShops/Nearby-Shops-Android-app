package org.nearbyshops.whitelabelapp.AdminCommon.ItemsDatabase.ItemsDatabaseForAdmin.ViewHolders;
...
public class ViewHolderItemAdmin extends RecyclerView.ViewHolder  implements PopupMenu.OnMenuItemClickListener{
    ...
    public void setItem(Item item) {
        ...
        priceRange.setText("Price Range :\n " + PrefCurrency.getCurrencySymbol(context) + String.format(" %.2f",itemStats.getMin_price()) + " - " + String.format("%.2f",itemStats.getMax_price()) + " per " + item.getQuantityUnit());
        priceAverage.setText("Price Average :\n " + PrefCurrency.getCurrencySymbol(context) + String.format(" %.2f",itemStats.getAvg_price()) + " per " + item.getQuantityUnit());
